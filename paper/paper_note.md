# ICRA 2011 - olson AprilTag： 一个健壮而灵活的视觉基准系统
...
> AprilTag和其他被动基准标记需要专门的算法来检测自然场景中其他特征中的标记。视觉处理步骤通常占据了标签检测流水线的计算时间，因此，即使在标记检测方面有很小的改进，也可以转化为更快的标签检测系统。我们将实现和支持AprilTag系统的经验教训整合到这个改进的svstem中。这项工作描述了AprilTag 2，一个完全重新设计的标签检测器，与原始的AprilTag系统相比，它提高了鲁棒性和效率。标签编码方案不变。对编码svstem固有的误报保持相同的鲁棒性。新的检测器以更高的检测率和更少的误报提高了性能。和更低的计算时间。改进的小图像性能允许使用抽取的输入图像。从而大大提高了检测速度。